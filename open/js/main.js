...
import { signer, provider, connect, sendTokens, onTokensReceived, txlink } from "./chainload";
...
async function onReceived(tx_hash, token_list) {
    ...
    const txAnchor = `https://mumbai.polygonscan.com/tx/${tx_hash}`// txlink(tx_hash);
...
try {
    const sendStarterKit = async function() {
        ...
        const amount = 1//parseInt(document.getElementById("token_amount").value);
