...
from doserve.config import TOKEN_DATA_DIRECTORY
...
class ChainLoader:
    ...
    def loadToken(self, tokenType, tokenId):
        tokenPath = self.targetPaths[tokenType]
        tokenFile = Path("%s/%s.json" % (tokenPath, tokenId))
        imagePath = "%s/%s.png" % (tokenPath, tokenId)
    ...
    def storeToken(self, tokenType, tokenId):
        ...
        if not tokenExists:  # and not imageExists:# and imageExists:
            print('Missing token %s' % tokenId)
            try:
                ...
                print('Failed to download token %s' % tokenId)
                if tokenFile.exists():
                    tokenFile.unlink()
                return False
        print('Token %s already exists' % tokenId)
        return True

def loadLocalNFT(type, tokenId):
    try:
        file = os.path.join(TOKEN_DATA_DIRECTORY, 'metadata', '{}.json'.format(tokenId))
        print('loading local data from %s' % file)
        with open(file, 'r') as f:
            data = json.load(f)
            return data
    except FileNotFoundError:
        print('Failed to load metadata for token %s' % tokenId)
        return False

def loadLocalNFTOpenSea(type, tokenId):
    try:
        file = os.path.join(TOKEN_DATA_DIRECTORY, 'opensea', '{}.json'.format(tokenId))
        print('loading local data from %s' % file)
        with open(file, 'r') as f:
            data = json.load(f)
            return data
...
if __name__ == "__main__":
    ...
    for i in range(1,10001):
        try:
            while not loader.storeToken('weapon', i):
